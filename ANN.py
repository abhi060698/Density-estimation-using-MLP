...
model = Sequential()
def monohint(y_true, y_pred,lamda):
            t1 = K.sum(K.square(y_true-y_pred))
            t2= 0
            N = model.input.get_shape().as_list()[-1]
            for j in range(1,N):
                for i in range(1,N):
                    if(i!=j):
                        t2+=K.sigmoid(model.input[:,j]-model.input[:,i])
            t2 = t2*(lamda/N-1)
            return t1+t2
def custom_activation(lamda):
    def dice(y_true,y_pred):
        return monohint(y_true, y_pred,lamda)
    return dice
custom = custom_activation(lamda=0.1)
...
class ANN:
    ...
    def __init__(self,X,Y,nodes,model=model,input_activation='relu',hidden_layers_activation='relu',output_activation='softmax',loss=custom,optimizer='adam',epochs=10,batch_size=200,kernel_init='normal'):#constructor
...
def test(): #perform a test to see if the code works using the MNIST dataset
    ...
    c=ANN(X_train,y_train,[784,10],loss='categorical_crossentropy')
